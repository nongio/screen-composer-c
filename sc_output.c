...
#include "screencomposer_renderer.h"
			   ...
			   struct sc_compositor *compositor)
{
	...
	wlr_output_init_render(output->wlr_output, compositor->wlr_allocator, compositor->wlr_renderer);
	...
	output->repaint_timer =
		wl_event_loop_add_timer(compositor->wl_event_loop, output_repaint_timer_handler, output);
...
//static void sc_output_send_frame_done(struct sc_output *output, struct send_frame_done_data *data) {
